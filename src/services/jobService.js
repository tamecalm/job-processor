...
export const jobService = {
  async createJob(name, data) {
    ...
    const queueJob = await createJobQueue.add(name, { ...data, id: job._id }, { jobId: job._id.toString() });
  ...
  async deleteJob(id) {
    ...
    await createJobQueue.remove(id);
    await job.remove();
  ...
  async retryJob(id) {
    ...
    const queueJob = await createJobQueue.add(job.name, { ...job.data, id: job._id }, { jobId: job._id.toString() });
